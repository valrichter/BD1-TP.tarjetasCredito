...
CREATE TABLE tarjeta(
    ...
    validahasta char(6),
    codseguridad char(4),
    limitecompra decimal(8,2),--solo puedo ingresar hasta 999.999
...
--para un consumo, tiene que existir una tarjeta y un comercio FK
...
ALTER TABLE consumo      ADD CONSTRAINT consumo_nrocomercio     FOREIGN KEY (nrocomercio)   REFERENCES comercio(nrocomercio);

--Funciones
CREATE OR REPLACE FUNCTION borrar_pks_fks() returns void as $$
	BEGIN
		--borra pks y fks en orden no dependiente, si borro primero cierre pk lanza error, otras dependen de esa pk
		...
		ALTER TABLE consumo DROP constraint consumo_nrocomercio;
...
CREATE OR REPLACE FUNCTION cargar_tablas() returns void as $$ 
	BEGIN
		PERFORM	cargar_clientes();
		PERFORM cargar_tarjetas();
		PERFORM cargar_comercios();
		PERFORM	cargar_cierres();
	END;
$$ LANGUAGE plpgsql;

CREATE OR REPLACE FUNCTION simular_consumos() returns void as $$ 
	BEGIN
	-- cliente 2
	-- cliente 2
	INSERT INTO consumo values ('4819499406355990', 'ABEQ', 1, 3000.00); --vigente, limite compra de tarjeta 9000, entra a compra con false
	INSERT INTO consumo values ('4819499406355990', 'ABEQ', 1, 10000.00); --vigente, limite compra de tarjeta 9000 -rechazo-
	INSERT INTO consumo values ('4819499406355990', 'ABEQ', 2, 7000.00); 
	--vigente pero tenia una compra de 3000 que aun no pago, entonces 3000+7000 supera el monto de 9000, rechazo
	INSERT INTO consumo values ('4819499406355990', 'ABEQ', 2, 8000.00); --rechazo
	INSERT INTO consumo values ('4819499406355990', 'ABEQ', 3, 2000.00); --este entra 3000+2000 = 5000 < 9000
	
	-- sin cliente
	INSERT INTO consumo values ('1234123412341234', 'ABEQ', 3, 2000.00); --rechazo, tarjeta invalida (no existe)

	-- cliente 6
	INSERT INTO consumo values ('6011511477319402', 'MNBJ', 7, 2000.00); --rechazo, tarjeta vencida
	INSERT INTO consumo values ('6011511477319402', 'MNBJ', 10, 8000.00); --rechazo, tarjeta vencida 

	-- cliente 17
	INSERT INTO consumo values ('3424221903251379', 'MXHE', 7, 2000.00); --rechazo, tarjeta anulada
						        
	-- cliente 1 (se ve el resumen)
	INSERT INTO consumo values ('5397853252415070', 'GPXL', 11, 10000.00); -- vigente, limite de 100000.90, entra a compra con false
	INSERT INTO consumo values ('5397853252415070', 'GPXL', 19, 50000.00); -- este entra 50000+10000 = 60000 < 100000.90
	INSERT INTO consumo values ('5397853252415070', 'GPXL', 5, 90000.00); -- rechazo
	INSERT INTO consumo values ('5397853252415070', 'GPXL', 6, 80000.00); -- rechazo
	INSERT INTO consumo values ('5397853252415070', 'GPXL', 5, 1000.00); -- entra, resumen =  61000

	-- cliente 14 (2 tarjetas) (se ve el resumen)
...
CREATE OR REPLACE FUNCTION cargar_clientes() returns void as $$
	BEGIN
		INSERT INTO cliente VALUES(1,'Juan','Pereira','Alfaro Rosas 1225','11-3570-6626');
		...
		INSERT INTO cliente VALUES(14,'Zulma','Ibañez','Versalles 2219','11-1824-0025'); -- tarjeta expirada (14)
		INSERT INTO cliente VALUES(15,'Luz','Espeche','Parque Chacabuco 721','11-5118-8851'); 
...
CREATE OR REPLACE FUNCTION cargar_tarjetas() returns void as $$
	BEGIN
		INSERT INTO tarjeta VALUES('5397853252415070', 1 , '201106', '202401', 'GPXL', 100000.90 ,'vigente');
...
CREATE OR REPLACE FUNCTION cargar_comercios() returns void as $$
	BEGIN
		INSERT INTO comercio VALUES(1,'coto', 'dean funes 5321', 'B1675', '11-4323-3212');
		...
		INSERT INTO comercio VALUES(13,'barber shop', 'alsina 2940', '9353', '11-8272-2344');
...
CREATE OR REPLACE FUNCTION cargar_cierres() returns void as $$
	...
	BEGIN		
		FOR mes IN 1 .. 12 LOOP --repite este codigo para los 12 meses del anio
			FOR terminacion IN 0 .. 9 LOOP --por cada terminacion crear una fecha con un dia de diferencia		
				INSERT INTO cierre VALUES(2021, mes, terminacion, finicio, fcierre, fvto);
				finicio := finicio + 1;
				fcierre := fcierre + 1;
				fvto := fvto + 1;
			END LOOP;
			finicio := (finicio - 9) + 30; --(un dia de intervalo entre cada fecha de cierre e inicio)
			fcierre := finicio + 30;
			fvto := fcierre + 11;
		END LOOP;		
	END;
...
BEGIN
		fhoy := now(); -- se supone que trae la fecha y hora actual
		SELECT * INTO tarjeta_ingresada FROM tarjeta WHERE nrotarjeta = i_n_tarjeta;
		
		IF NOT FOUND THEN -- Si no existe la tarjeta
			INSERT INTO tarjeta VALUES (i_n_tarjeta, 1, null, null, null, null, 'anulada');-- xq nro cliente = 1?
																						   -- nro cliente 1 por default	pq es FK y no puede ser null
		...
		-- Si el la tarjeta tiene un codigo de seguridad invalido, existe
		...
		-- Si la tarjeta se encuentra suspendida
		IF tarjeta_ingresada.estado = 'suspendida' THEN
			 INSERT INTO rechazo (nrotarjeta, nrocomercio, fecha, monto, motivo)
			 VALUES (i_n_tarjeta, i_n_comercio, fhoy, i_monto, 'la tarjeta se encuentrs suspendida');
			 RETURN false;
		END IF;
...
BEGIN
		...
		--para cada tarjeta que tenga el cliente generamos su resumen
		FOR tarjetaAux IN SELECT * FROM tarjeta WHERE nrocliente = i_nro_cliente loop
			...
			--recorremos todas las compras que hizo esa tarjeta
			FOR compraAux IN SELECT * FROM compra WHERE nrotarjeta = tarjetaAux.nrotarjeta AND pagado = false loop
			
				--verificamos que las compras sean del mes y del anio pasado por parametro
			--	IF i_año = date_part('year', compraAux.fecha) AND i_mes = date_part('month', compraAux.fecha) THEN
			fAux = compraAux.fecha::date ;
				IF fAux <= cierreAux.fechacierre and fAux > cierreAux.fechainicio THEN
					--calculamos el lo que gasto en ese periodo
			...
			--insertamos los datos en la tabla cabecera
			...
			--obtenemos el ultimo nroresumen de cabecera
			...
			FOR compraAux IN SELECT * FROM compra WHERE nrotarjeta = tarjetaAux.nrotarjeta AND pagado = false loop
				...
				IF fAux <= cierreAux.fechacierre and fAux > cierreAux.fechainicio THEN
					...
					--obtengo el nombre del comercio
...
CREATE OR REPLACE FUNCTION alertas_a_clientes() returns TRIGGER as $$
DECLARE
	alertaAux record;
	cantRechazos int = 0;
	
BEGIN
	--cuenta las veces que se rechazo esa tarjeta por limite de compra en un dia	
	...
	--si la rechazaron dos o mas vece por limite de compra	
...
CREATE TRIGGER alertas_a_clientes_trg
AFTER INSERT on rechazo
FOR each row
execute PROCEDURE alertas_a_clientes();


CREATE OR REPLACE FUNCTION alertas_a_compras() returns TRIGGER as $$
...
BEGIN	
	--para cada compra de esa tarjeta se fija...	
	FOR compraAux IN SELECT * FROM compra WHERE nrotarjeta = new.nrotarjeta loop			
		--calculo en tiempo entre compras		
		tiempo := EXTRACT(EPOCH FROM (compraAux.fecha - new.fecha::timestamp));	

		--obtengo el comercio al cual pertenece la compraAux	
		SELECT * INTO comercioAux FROM comercio WHERE nrocomercio = compraAux.nrocomercio;	
				
		--obtengo el comercio al cual pertenece la compraNew		
		SELECT * INTO comercioNew FROM comercio WHERE nrocomercio = new.nrocomercio;	
				
		-- si entre la compra actual y alguna de las compras anterios es menor a 60 (1 min) segundos AND	
		-- son comercios distintos AND		
		-- estan ubicados en el mismo cosdigo postal	
		IF tiempo < 60 AND comercioAux.nrocomercio != comercioNew.nrocomercio AND comercioAux.codigopostal = comercioNew.codigopostal THEN
			INSERT INTO alerta (nrotarjeta, fecha, nrorechazo, codalerta, descripcion)			
			VALUES (new.nrotarjeta, new.fecha, null, 1, 'compra 1min');					
		END IF;
				
		-- si entre la compra actual y alguna de las compras anterios es menor 5min segundos AND		
		-- son comercios ubicados en distintos lugares	
...
PROCEDURE alertas_a_compras();

--
SELECT cargar_tablas(); -- carga la tablas con los datos automaticamente
SELECT simular_consumos();-- carga los consumos
SELECT simular_pasar_consumos_a_compra_o_rechazo(); -- decide si es una compra o un rechazo (sirve para testear autorizar_compra tambien)
SELECT generar_resumenes();
